...
const getVoterByName = (nameOfVoter, boxNumber, cb) => {
  const queryString = 'SELECT id,full_name,serial_box_number FROM voters WHERE box_number=$1 AND full_name LIKE "%$2%"';
  dbConnection.query(queryString, [boxNumber, nameOfVoter], cb);
};

const getVoterByIdOrSerial = (idOrSerial, boxNumber, cb) => {
  const queryString = 'SELECT id,full_name,serial_box_number FROM voters WHERE box_number=$1 AND (id::text LIKE "%$2%" OR serial_box_number::text LIKE "%$2%")';
